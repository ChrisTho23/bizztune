...
def format_ticket(ticket, hide_output=False):
    ...
    formatted_text = (
        ...
        f"Name: {input_data.get('name', 'N/A')}\n"
        f"Date: {input_data.get('date', 'N/A')}\n"
        f"Urgency: {input_data.get('urgency', 'N/A')}\n"
    )
...
def display_example(example, model=None, predicted_category=None, predicted_subcategory=None):
